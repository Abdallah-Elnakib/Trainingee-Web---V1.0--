 ...
 */
function evaluateAnswerWithAI(answer, question, maxPoints) {
...
Devuelve un JSON con este formato exacto: {"score": [puntuación numérica entre 0 y 100], "feedback": "[retroalimentación detallada para el estudiante]"}`;
            ...
            const response = yield axios_1.default.post('https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.2', { inputs: prompt }, {
                headers: {
 ...
 */
function fallbackEvaluation(answer, question, maxPoints) {
    ...
    const matchingKeywords = questionKeywords.filter(keyword => answerKeywords.includes(keyword) || normalizedAnswer.includes(keyword));
...
const submitQuestionAnswer = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let track = yield tracksSchema_1.Track.findOne({ trackName });
        ...
        if (Array.isArray(studentTrack.tasks)) {
            studentTask = studentTrack.tasks.find((t) => {
                if (!t || !t.taskName)
                    return false;
        ...
        const evaluationResult = yield evaluateAnswerWithAI(answer, question || 'No question provided', pointsPerQuestion);
        ...
        if (track && Array.isArray(track.trackData)) {
            ...
            const studentInTrack = track.trackData.find((std) => std && std.Name === student.name);
            if (studentInTrack) {
                // Buscar la tarea en BasicTotal
                const taskInBasicTotal = Array.isArray(studentInTrack.BasicTotal) ?
                    studentInTrack.BasicTotal.find((t) => t && t.taskName === task.taskName) : null;
                if (taskInBasicTotal) {
